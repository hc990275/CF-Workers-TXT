...
async function getContent(env) {
  const current = await env.KV.get(CONTENT_FILE);
  if (current !== null) return current;

  const legacy = await env.KV.get(LEGACY_CONTENT_FILE);
  if (legacy !== null) {
...
async function saveContent(env, data) {
  await env.KV.put(CONTENT_FILE, data);
  await env.KV.put(LEGACY_CONTENT_FILE, data);
}
